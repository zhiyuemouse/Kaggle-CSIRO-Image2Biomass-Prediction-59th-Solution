...
oof_path = [
    "CSIRO/output/2026-01-15_23:00:56_vit_large_patch16_dinov3.lvd1689m_output/oof.npy", # 8610
    "CSIRO/output/2026-01-15_22:56:47_vit_large_patch16_dinov3.lvd1689m_output/oof.npy", # 8533
]
...
if __name__ == "__main__":
    oof = None
    # for _path in oof_path:
    #     if oof is None:
    #         # oof = np.load(_path)
    #         oof = apply_mass_balance_numpy(np.load(_path))
    #     else:
    #         # oof += np.load(_path)
    #         oof += apply_mass_balance_numpy(np.load(_path))
    oof1 = apply_mass_balance_numpy(np.load(oof_path[0]))
    oof2 = apply_mass_balance_numpy(np.load(oof_path[1]))
    oof = 0.7 * oof1 + 0.3 * oof2
    # oof = oof / len(oof_path)
